-- Seed Data for Countries, Counties, and Sub-Counties
...
DECLARE
    ...
    finland_id UUID;
    county_rec RECORD;
    sub_county_text TEXT;
    
    -- Helper function to insert county and its subcounties
    -- We use a temporary table or just logic within the loop
BEGIN
    -- 1. Insert Countries
    INSERT INTO public.countries (name) VALUES ('Kenya') ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id INTO kenya_id;
    INSERT INTO public.countries (name) VALUES ('Tanzania') ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id INTO tanzania_id;
    INSERT INTO public.countries (name) VALUES ('Finland') ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id INTO finland_id;

    -- 2. Insert Counties and Sub-Counties for Kenya
    -- We will use a loop for cleaner code. 
    -- Format: County Name | Comma separated sub-counties
    
    FOR county_rec IN SELECT * FROM (VALUES 
    ...
    ) AS t(county_name, sub_counties_str)
    LOOP
        DECLARE
            new_county_id UUID;
            sc_name TEXT;
        BEGIN
            ...
            VALUES (county_rec.county_name, kenya_id)
            ON CONFLICT (name, country_id) DO UPDATE SET name = EXCLUDED.name
            RETURNING id INTO new_county_id;

            -- Insert Sub-Counties
            FOREACH sc_name IN ARRAY string_to_array(county_rec.sub_counties_str, ',')
            LOOP
                INSERT INTO public.sub_counties (name, county_id)
                VALUES (trim(sc_name), new_county_id)
